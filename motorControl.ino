...
void loop() {
  if (Serial.available() > 0) {
    ...
    if(h < 170)
    {
      digitalWrite(motor2pin1, HIGH); // clock wise
      digitalWrite(motor1pin1, HIGH); // clock wise
      analogWrite(10, 150);  // Set speed using PWM, motor1
      analogWrite(9, 150);  // Set speed using PWM, motor2
      delay(10);
      // if wanting to turn right
      if (x_pos > 500)
      {
      ...
      {
        digitalWrite(motor2pin1, LOW); // clock wise
        delay(10);
      }
    }
    ...
    {
      digitalWrite(motor2pin2, HIGH); // counter clock wise 
      digitalWrite(motor1pin2, HIGH); // counter clock wise
      analogWrite(10, 150);  // Set speed using PWM, motor1
      analogWrite(9, 150);  // Set speed using PWM, motor2
      delay(10);
      if (x_pos > 500)
